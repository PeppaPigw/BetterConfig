...
describe('runApp snapshot switching', () => {
  it('switches between original and latest generated snapshots', async () => {
    const root = await mkdtemp(path.join(tmpdir(), 'betterconfig-snapshots-'));
    ...
    const secondRun = await runApp({
      ...
      codexAdapter: {
        detect: async () => ({ installed: true, installHelp: [] }),
        checkAuth: async () => ({ status: 'authenticated', details: 'ok' }),
        runOfficialLogin: async () => ({ exitCode: 0, stdout: '', stderr: '' }),
      },
    });

    const finalConfig = await readFile(configPath, 'utf8');
    expect(secondRun.status).toBe('exited');
